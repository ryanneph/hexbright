...
void update_spin() {
  ...
  if(!i) {
    ...
    Serial.println("/" + (String)((int)(UPDATE_DELAY*1000)));
  }
  last_time = now;
#endif
  if(now-continue_time>5000 && !i) {
    ...
    Serial.println("WARNING: code is too slow " + (String)(now-continue_time));
  }
  if (!i)
...
void update_spin() {
  ...
  unsigned long start_time=micros();
	  ...
	  while (true) {
		  do {
			  now = micros();
		  } while (((next_strobe > now) || strobe_off_time > now) && // not ready for strobe
			  continue_time > now); // not ready for update); // strobe off or strobe not ready to go dark yet
		  if (strobe_state == true && strobe_off_time <= now) { //the light is has been on for 'strobe_duration' and needs to be set to dark until next strobe
			  digitalWriteFast(DPIN_DRV_MODE, LOW);
			  digitalWriteFast(DPIN_DRV_EN, LOW);
			  strobe_state = false;
				  //Serial.println("strobe_off_time Met");
				  //Serial.println("now: " + (String)now);
				  //Serial.println("off: " + (String)strobe_off_time);
		  }
		  if (next_strobe <= now) {
			  ...
			  //Serial.println("H:" + (String)strobe_off_time + ":" + (String)now + ":" + (String)next_strobe);
